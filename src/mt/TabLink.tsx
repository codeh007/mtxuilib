...
const tabLinksVariants = cva("", {
  variants: {
    layout: {
      default: "inline-flex h-9 w-fit items-center justify-center rounded-lg p-1",
...
export function TabLinks({ className, layout = "underlined", children }: TabLinksProps) {
  return (
...
export function TabLinkItem({ className, variant = "underlined", href, children, ...props }: TabLinkItemProps) {
  // const location = useLocation();
  // const isActive = location.pathname === to || location.pathname.startsWith(`${to}/`);

  return (
    <Link href={href} className={cn(tabLinkItemVariants({ variant }), className)} {...props}>
      {children}
    </Link>
  );
}
