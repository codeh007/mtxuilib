...
import { CheckCircle, Clock, Loader2, Play, Square, XCircle } from "lucide-react";
...
export function StatusIcon({ status, className, size = "md" }: StatusIconProps) {
  ...
  return (
    <Icon
      className={cn(statusIconVariants({ size, status: config.variant }), config.animate && "animate-spin", className)}
    />
  );
}
