...
const CodeHighlighter = dynamic(() => import("./code-highlighter").then((mod) => mod.CodeHighlighter), {
  loading: () => <div className="p-8 text-xs text-muted-foreground text-center animate-pulse">Loading viewer...</div>,
  ssr: false,
});
...
const createDebugSlice: StateCreator<MtmaiState, [], [], MtmaiState> = (set, _get, init) => {
  return {
    debug: false,
    ...init,
...
export const DebugProvider = (props: AppProviderProps) => {
  ...
  return <mtmaiStoreContext.Provider value={store}>{children}</mtmaiStoreContext.Provider>;
};
...
export function DebugValue({ title, data, className }: PropsWithChildren<DebugValueProps>) {
      ...
      <DialogTrigger asChild>
        <div className="inline-flex align-middle">
          <Tooltip>
            ...
            <TooltipContent side="top" className="text-xs max-w-[200px] break-all">
              Inspect: {title || "Data"}
            </TooltipContent>
          </Tooltip>
        </div>
