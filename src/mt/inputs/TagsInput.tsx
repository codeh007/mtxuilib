...
interface TagsInputProps extends Omit<React.ComponentProps<typeof Input>, "value" | "onChange"> {
  value?: string[];
...
export function TagsInput({
  ...
  return (
    ...
    >
      {tags.map((tag, index) => (
        <Badge key={`${tag}-${index}`} variant="secondary" className="gap-1 pr-1 text-xs">
          {tag}
          <button
            type="button"
