...
export const SkeletonLoadingSpinner = ({ className }: BaseLoadingProps) => {
  return (
    <div className={cn("w-full mx-auto h-full flex justify-center items-center flex-1", className)}>
      <Spinner />
    </div>
  );
};
...
export function InputSkeleton({ className }: BaseLoadingProps) {
  return <Skeleton className={cn("h-9 w-full rounded-md border px-3 py-1 shadow-sm", className)} />;
}
...
export function LayoutSkeleton({
  ...
  return (
    ...
    >
      ...
      {text && <div className="mt-4 text-center text-muted-foreground">{text}</div>}
    </div>
  );
}
...
export function TableLoadingSkeleton({ rows = 5, showHeader = true, className }: TableLoadingSkeletonProps) {
  return (
    <div className={cn("flex flex-col space-y-4 p-4", className)}>
      {showHeader && <Skeleton className="h-8 w-1/3 mb-6" />}
      <div className="space-y-2">
        {Array.from({ length: rows }, (_, i) => (
          <Skeleton key={`table-skeleton-${Date.now()}-${i}`} className="h-10 w-full" />
        ))}
      </div>
    </div>
  );
...
export function CardLoadingSkeleton({ columns = 2, cardHeight = "200px", className }: CardLoadingSkeletonProps) {
  const gridClasses = {
    1: "grid-cols-1",
    ...
    <div className={cn("flex flex-col space-y-4 p-4", className)}>
      ...
      <div className={cn("grid gap-4", gridClasses[columns])}>
        {Array.from({ length: columns * 2 }, (_, i) => (
          <Skeleton key={`card-skeleton-${Date.now()}-${i}`} className="w-full" style={{ height: cardHeight }} />
        ))}
      </div>
    </div>
  );
...
export function PageLoadingSkeleton({
  ...
  return (
    <div className={cn("space-y-6 p-6", className)}>
      ...
      {showContent && (
        <div className="space-y-4">
          {contentType === "table" && <TableLoadingSkeleton rows={contentRows} showHeader={false} />}
...
export function SkeletonLoading({ className }: BaseLoadingProps) {
  return (
    <div className={cn("mx-auto flex h-full w-full flex-col items-center justify-center rounded-md", className)}>
